...
def main(args: argparse.Namespace):
    ...
    reached_goal = False
     ...
     # ROS
    rclpy.init_node("EXPLORATION", anonymous=False)
    rate = rclpy.Rate(RATE)
    image_curr_msg = rclpy.Subscriber(
        IMAGE_TOPIC, Image, callback_obs, queue_size=1)
    waypoint_pub = rclpy.Publisher(
        WAYPOINT_TOPIC, Float32MultiArray, queue_size=1)  
    sampled_actions_pub = rclpy.Publisher(SAMPLED_ACTIONS_TOPIC, Float32MultiArray, queue_size=1)
    goal_pub = rclpy.Publisher("/topoplan/reached_goal", Bool, queue_size=1)
