...
def callback_joy(msg: Joy):
    if msg.buttons[0]:
        rclpy.signal_shutdown("shutdown")


def main(args: argparse.Namespace):
    global obs_img
    rclpy.init_node("CREATE_TOPOMAP", anonymous=False)
    image_curr_msg = rclpy.Subscriber(
        IMAGE_TOPIC, Image, callback_obs, queue_size=1)
    subgoals_pub = rclpy.Publisher(
        "/subgoals", Image, queue_size=1)
    joy_sub = rclpy.Subscriber("joy", Joy, callback_joy)
